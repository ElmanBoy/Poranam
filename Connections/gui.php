...
function buildSelectFromRegistry($items, $selected = [], $firstEmpty = false): string
{
...
function getGroupByUser($userDataArray, $showAll = false): array
{
    $g = null;
    $out = [];
    $query = '';
    //По рангу пользователя собираем подзапрос
    switch ($userDataArray['field6']){

        case 10: //Пользователь
            $query = " AND g.field1 = 'И-".$userDataArray['field11']."'";
            break;
        case 9://Куратор индекса
            $query = " AND g.field1 = 'Р-".$userDataArray['field11']."'";
            break;
        case 8: //Куратор района
            $query = " AND g.field1 = 'Н-".$userDataArray['field8']."'";
            break;
        case 7: //Куратор нас. пункт
            $query = " AND g.field1 = 'С-".$userDataArray['field8']."'";
            break;
        case 6: //Куратор субъекта
            $query = " AND g.field1 = 'Г'";
            break;
        case 5: //Куратор страна
            $query = " AND g.field1 = 'Ц'";
            break;
        case 4: //Куратор центра
            $query = " AND g.field1 = 'А'";
            break;
...
field8 Субъект РФ
field9 Регионы
field10 Населённый пункт
field11 Индекс
field12 Улица
field13 Номер дома
field16 Группа*/

    $g = el_dbselect("SELECT g.field1 AS user_index, g.field2 AS group_number, g.id AS group_id
    FROM catalog_groups_data g 
    WHERE (SELECT COUNT(id) FROM catalog_users_data WHERE field16 = g.id) <= 50 $query 
    GROUP BY g.id ORDER BY g.id",
        0, $g, 'result', true);
    if(el_dbnumrows($g) > 0) {
        $rg = el_dbfetch($g);
        do {
            $out[$rg['group_id']] = ($rg['user_index'] == 'А')
                ? $rg['user_index'] : $rg['user_index'] . '-' . $rg['group_number'];
        } while ($rg = el_dbfetch($g));
    }
    return $out;
}

function getGroupsByUser($userDataArray, $showAll = false): array
{
    $g = null;
    $out = [];
    $query = '';
...
field16 Группа*/
    ...
    $g = el_dbselect("SELECT g.field1 AS user_index, g.field2 AS group_number, g.id AS group_id
    FROM catalog_groups_data g 
    WHERE (SELECT COUNT(id) FROM catalog_users_data WHERE field6 = g.id) < 50  $query 
    GROUP BY g.id ORDER BY g.id",
        0, $g, 'result', true);
    if(el_dbnumrows($g) > 0) {
        $rg = el_dbfetch($g);
