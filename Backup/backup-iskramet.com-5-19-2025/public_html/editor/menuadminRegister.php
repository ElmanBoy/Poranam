...
<button title="Добавить новый раздел в меню"
        onClick="MM_openBrWindow('newcategory.php?parentid=0&site_id=<?=$_SESSION['site_id']?>&cat_type=3','newcat','scrollbars=no,resizable=yes','720','500',
                'true')">
...
{//Child Items
    static $level;
    global $userLevel;
    $querymenuchild = "SELECT * FROM cat WHERE parent='$parent' AND cat_type = 3 ORDER BY sort ASC";
...
{//Parent items, first level only
    ...
    $querymenutree = "SELECT * FROM cat WHERE cat_type = 3 AND site_id = 1 ORDER BY sort ASC";
            ...
            if (in_array($userLevel, $araccess) || $userLevel == "1") {
                ...
                <li class="parent" id="tr<?= $row_menutree['id']; ?>">
                    ...
                    <div class="parent4">
                        <table border="0" cellpadding="0" cellspacing="0" width="100%">
                            <tr>
                                <td align="center" class="lbr"><a href="editor.php?cat=<?php echo $row_menutree['id']; ?>"
                                                                  target="Main"
                                                                  title="Редактировать содержимое раздела">
                                        <i class="material-icons">edit</i></a>
                                </td>
