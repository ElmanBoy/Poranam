/*CKEDITOR.dialog.add( 'galleryImage', function( editor )
{
	return {
		title : 'My Dialog',
		minWidth : 400,
...
(function()
{
	...
	var switchLockRatio = function( dialog, value )
	{
		...
		if ( oImageOriginal.getCustomData( 'isReady' ) == 'true' )
		{
			if ( value == 'check' )			// Check image ratio and original image ratio.
			{
				...
				height=Math.round(150/thisRatio);
	...
	var resetSize = function( dialog )
	{
		...
		oImageOriginal.$.width=(oImageOriginal.$.width>150)?150:oImageOriginal.$.width;
		oImageOriginal.$.height=Math.round(150/originalRatio);
		imgSrc=CKEDITOR.tools.trim( dialog.getValueOf( 'info', 'txtUrl' ) );
		imgAlt=CKEDITOR.tools.trim( dialog.getValueOf( 'info', 'txtAlt' ) );
	...
	var imageDialog = function( editor, dialogType )
	{
		...
		return {
			title : 'Свойства изображения для галереи',
			minWidth : 520,
			minHeight : 310,
			onShow : function()
			{
				...
				this.originalElement.setAttribute( 'rel', 'lightbox[roadtrip]' );
							...
							commit : function( type, element )
							{
								if ( type == LINK )
								{
									if ( this.getValue() || this.isChanged() )
										...
										element.setAttribute( 'rel', 'lightbox[roadtrip]' );
										element.setAttribute( 'href', imgSrc );
										element.setAttribute( 'title', imgAlt );
								}
							}
						}
					]
