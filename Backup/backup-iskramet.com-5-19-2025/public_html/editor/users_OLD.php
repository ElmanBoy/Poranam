	...
	if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "user")) {
		...
		if(mysqli_num_rows($users)>0&&$row_users['primary_key']!=$_POST['id']){
			...
			if(strlen($_POST['pass2'])>0){
  			...
  			$updateSQL = sprintf("UPDATE phpSP_users SET `user`=%s, password=%s, userlevel=%s, fio=%s, email=%s, birthday=%s, INN=%s, phones=%s, fsource=%s, post_adress=%s, dev_adress=%s WHERE primary_key=%s",
...
;
$query_status = "SELECT * FROM userstatus ORDER BY `level` ASC";
...
<table width="90%" border="0" align="center" cellpadding="3" cellspacing="0" class="el_tbl">
  ...
  <tr>
    <td>Дата рождения:</td>
    <td><input name="birthday2" type="text" id="birthday2" value="<?php echo $row_users['birthday']; ?>" size="40"></td>
  </tr>
  <tr>
    <td>E-mail:</td>
    <td><input name="email2" type="text" id="email2" value="<?php echo $row_users['email']; ?>" size="40"></td>
  </tr>
  <tr>
...
<table width="100%"  border="0" align="center" cellpadding="5" cellspacing="0" class="el_tbl">
      <form method="POST" action="<?php echo $editFormAction; ?>" name="newuser">
        ...
        <tr>
          ...
          <td><input name="birthday" type="text" id="birthday" value="<?=$_POST['birthday']?>">
            (дд.мм.гггг) </td>
        </tr>
        <tr>
