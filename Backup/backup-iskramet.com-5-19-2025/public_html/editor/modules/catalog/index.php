...
if (!isset($_REQUEST['ajax'])){
    ...
    //Собираем справочник путей
    $c = el_dbselect("SELECT cat.id AS id, cat.path AS path, catalog_" . $catalog_id . "_data.cat AS cat FROM catalog_" . $catalog_id . "_data, cat 
WHERE cat.id = catalog_" . $catalog_id . "_data.cat GROUP BY catalog_" . $catalog_id . "_data.cat", 0, $c, 'result', true);
    $rc = el_dbfetch($c);
    $catsPath = array();
    do {
    ...
    $title_catalog = el_dbselect("SELECT field FROM catalog_prop WHERE title='1' AND catalog_id='$catalog_id' ORDER BY field ASC", 1, $title_catalog, 'row');
	?>
...
<link rel="stylesheet" href="/js/fine-uploader/fine-uploader-gallery.css">
	<script language="javascript">
        ...
        function selectMode(divId) {
            ...
            for (var i = 0; i < p.length; i++) {
                ...
                document.cookie = "idshow[" + p[i].id + "]=N; expires=Thu, 31 Dec 2120 23:59:59 GMT; path=/editor/;";
            }
            d.style.display = 'block';
            document.cookie = "idshow[" + divId + "]=Y; expires=Thu, 31 Dec 2120 23:59:59 GMT; path=/editor/;";
        }
        ...
        function showChildCat(obj, level, parentId) {

            if ($(obj).attr('src') == '/editor/img/plus.gif') {
                $(obj).addClass('imgLoading');
                $.post('/editor/modules/catalog/getChildCats.php', {
                        parent: parentId,
                        catalog_id: '<?=$catalog_id?>',
                        level: level
                    },
                    function (data) {
                        $('#cat' + parentId).after(data);
                        $(obj).attr('src', '/editor/img/minus.gif');
                        $(obj).removeClass('imgLoading');
                    })
            } else {
	...
	<style type="text/css">
        ...
        .imgLoading {
            background: url('/editor/img/loading.gif') no-repeat;
            background-size: 9px;
        }
...
//Сохранение параметров отображения каталога
	...
	if (isset($_GET['frm_filter']) && $_GET['frm_filter'] == 'search') {
		...
		$qw = "";
		do {
			if (!is_array($_GET['field' . $row_pfilter['field']]) && strlen($_GET['field' . $row_pfilter['field']]) > 0) {
...
if (!$mode){
	...
	$catQuery = (strlen($qw) == 0) ? "(cat = $cat  OR cat LIKE '% $cat %') AND " : "";


	$query_catalog = "SELECT * FROM catalog_" . $catalog_id . "_data 
	WHERE $catQuery site_id=" . intval($_SESSION['site_id']) . $qw . " ORDER BY$sort";//
	...
	$row_dbcontent['path'] = '/editor/modules/catalog';
...
<p><br><a name="start_list"></a>
	...
	Показано c <?php echo($startRow_catalog + 1) ?> по <?= ($maxRows_catalog > 0) ? min($startRow_catalog + $maxRows_catalog, $totalRows_catalog) : $totalRows_catalog ?> из <?php echo $totalRows_catalog ?> записей
	<? if ($maxRows_catalog > 0) el_paging1($pageNum_catalog, $currentPage, $queryString_catalog,
	$totalPages_catalog, $maxRows_catalog, $totalRows_catalog, 'cat=' . $cat . '&pageNum_catalog', 'tr') ?>
	</p>
	<div id="catalog_sort">
			...
			<tr id="string<?php echo $row_cat_formalog['id']; ?>"
				...
				<td width="50%" align="right">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<? if (is_array($price_field) && count($price_field) > 0) {
							...
							<td width="30"><a
										href="<?= $catsPath[intval($row_cat_formalog['cat'])] . '/' . $row_cat_formalog['path'] ?>.html"
										target="_blank"><i class="material-icons"
							                               title="Открыть на сайте">preview</i></a>
							</td>
							...
							<td width="30" id="td<?= $row_cat_formalog['id'] ?>"><i class="material-icons"
							                                                        ...
							                                                        onMouseOut="document.getElementById('td<?= $row_cat_formalog['id'] ?>').style.color=''">
									edit</i></td>
							<td width="30"><i class="material-icons" title="Сохранить изменения"
							                  onclick="$(this).parents('form').trigger('submit')">save</i></td>
...
<p>
	...
	<div id="addForm">
	   ...
	   FROM content, cat WHERE content.cat=cat.id AND content.kod='catalog" . $catalog_id . "' AND cat.parent=
	   ...
	   AND cat.site_id=" . intval($_SESSION['site_id']) . " ORDER BY cat.sort", 0, $currCat, 'result', true);
					...
					<div id="cat" style="max-height:300px; overflow:auto">
						<? do {
							...
							if ($child['count'] > 0) {
								?>
								<img src="/editor/img/plus.gif" class="icon_plus"
								     onClick="showChildCat(this, 1, <?= $currCat['cat'] ?>)">
							<? } ?>
							<label for="catCh<?= $currCat['cat'] ?>" id="cat<?= $currCat['cat'] ?>">
								<input type="checkbox" name="cats[]" id="catCh<?= $currCat['cat'] ?>"
								       value="<?= $currCat['cat'] ?>"<?= $sel ?>> <?= $currCat['title'] ?></label><br>
							<?
						...
						echo '</div>
							...
							echo '<input type="hidden" name="cats[]" value="' . $_GET['cat'] . '">';
						}
		  ...
		  <input type="checkbox" name="field' . $row_cat_form['field'] . '[]" id="opt' . $i . '" value="' . $itemsArr[$i] . '" checked="checked"> ' . $itemsArr[$i] . '</label><br>';
						...
						<tr>
							<td colspan="2" align="center" valign="top"><input type="submit" name="Submit"
							                                                   value="Добавить"
							                                                   class="but"></td>
						</tr>
...
if (isset($_REQUEST['ajax'])) {
    ...
    if(strlen($_REQUEST['setActive']) > 0){
        ...
        $res = el_dbselect("UPDATE catalog_".$catalog_id."_data SET active=$active WHERE 
        id=$id AND site_id='".$_SESSION['site_id']."'", 0, $res, 'result', true);
        if($res != false){
