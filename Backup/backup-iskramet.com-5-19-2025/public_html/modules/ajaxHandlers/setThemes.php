...
if (el_checkAjax()) {
    ...
    $new_theme = array('active' => 1, 'cat' => 408, 'field1' => $_POST['new_theme']);
    el_dbinsert("catalog_registryVote_data", $new_theme);

    foreach ($_POST as $name => $value) {
        if (substr_count('name', $name) > 0 && $name != 'new_theme') {
            $id = intval(str_replace('name', '', $name));
            el_dbselect("UPDATE catalog_registryVote_data SET field1 = '$value' WHERE id=$id",
                0, $res, 'result', true);
        }
    }
