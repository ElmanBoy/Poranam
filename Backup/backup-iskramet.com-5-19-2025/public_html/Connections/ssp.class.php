...
class SSP {
	...
	static function simple ( $request, $conn, $table, $primaryKey, $columns )
	{
		...
		if(is_array($request['params']) && count($request['params']) > 0){
		    $where .= (strlen($where) > 0 ? ' AND ' : ' WHERE ') . implode(' AND ', $request['params']);
        }
/*echo 'SELECT `' . implode('`, `', self::pluck($columns, 'db')) . "`
	...
	static function sql_connect ( $sql_details )
	{
		try {
            ...
            $db->setAttribute(PDO::MYSQL_ATTR_INIT_COMMAND, 'SET NAMES utf8; SET CHARACTER SET utf8; SET SESSION collation_connection = utf8_general_ci;');
		}
