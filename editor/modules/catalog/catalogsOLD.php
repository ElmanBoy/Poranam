...
$totalRows_cats = mysqli_num_rows($cats);
	if($totalRows_cats<1){ echo "<h4 align='center'>Нет ни одного каталога</h4>";}else{?>
<table width="80%" border="0" align="center" cellpadding="5" cellspacing="0" class="el_tbl">
  ...
  <? do{?>
  <tr><form method="post" action="<?=$_SERVER['REQUEST_URI']?>">
    <td width="50%" id="string<?=$row_cats['id']; ?>" style="color:#003399" onMouseOver='document.getElementById("string<?=$row_cats['id']; ?>").style.backgroundColor="#DEE7EF"' onMouseOut='document.getElementById("string<?=$row_cats['id']; ?>").style.backgroundColor="#CCDCE6"'>
	...
	<a href="?cid=<?=$row_cats['catalog_id']?>"><img src="/editor/img/leftmenu_tools.gif" alt="Настройки каталога" border="0"></a>
	</td>
    <td>
	<a href="?id=<?=$row_cats['catalog_id']?>"><img src="/editor/img/menu_edit.gif" alt="Редактировать структуру" width="23" height="17" border="0"></a>
	</td>
    <td>
	<input name="submit_save" type="image" id="submit_save2" src="/editor/img/menu_save.gif" alt="Сохранить изменения" onClick="cat_mode.value='save';">
	</td>
    <td>
	<input name="submit_del" type="image" id="submit_del" src="/editor/img/menu_delete.gif" alt="Удалить каталог" onClick="cat_mode.value='del';return check('<?=$row_cats['name']?>'); ">
	</td>
  </form></tr>
...
if((isset($_GET['id']) || isset($_GET['edit_cat'])) && $_GET['mode']!="new"){ 
  ...
  if(strlen($id_cat)<1){$id_cat=$_POST['catalog_id'];}
...
$catdata=el_dbselect($query_catdata, 0, $catdata);
	...
	if($_POST['cat_mode_edit1']=="save"){
		...
		$row_catsave = el_dbfetch($catsave);
		do{
	  		 $updateSQL = sprintf("UPDATE catalog_prop SET name=%s, sort=%s, title=%s, list=%s, detail=%s, search=%s, show_name=%s, inform=%s, `required`=%s WHERE id=%s",
	...
	if((isset($_POST['new_element']))&&($_POST['new_element']=="1")){ //print_r($_POST);
		for($i=0; $i<count($_POST['type']); $i++){
			...
			$insertSQL = sprintf("INSERT INTO catalog_prop (option_id, name, type, size, cols, rows, sort, title, list, detail, search, show_name,  inform, `required`, catalog_id, field, options, listdb, from_field) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
		  ...
		  switch ($option['type']){
			...
			$query_catdata = "ALTER TABLE catalog_".$id_cat."_data ADD `$name` $type NULL";
			$catdata=el_dbselect($query_catdata, 0, $catdata);
			el_reindex("catalog_".$id_cat."_data");
	}
}
...
<caption>Свойства каталога <b>"<?=$row_cats['name']?>"</b></caption>
  ...
  <tr>
    ...
    <td align="center" style="background-color:#b1c5d2">Сортировка <img src="/editor/img/help_button.gif" alt="Кликните для получения справки" width="12" height="12" style="cursor:pointer" onClick="test.TextPopup ('Этот параметр показывает, будет ли производиться сортировка по этому полю',pszFont,10,10,-1,-1)"></td>
    <td align="center" style="background-color:#b1c5d2">Поиск <img src="/editor/img/help_button.gif" alt="Кликните для получения справки" width="12" height="12" style="cursor:pointer" onClick="test.TextPopup ('Этот параметр показывает, будет ли по этому полю производиться расширенный поиск',pszFont,10,10,-1,-1)"></td>
...
($row_cat['required']==1)?$chek5="checked":$chek5="";
	?>
      ...
      <nobr><input name="name<?=$row_cat['id']?>" type="text" id="name<?=$row_cat['id']?>" value="<?=$row_cat['name']?>" size="30">
      </nobr><div style="text-align:center; font-size:10px; color:#006600">Имя поля: field<?=$row_cat['field']?></div></td>
    <td align="center" valign="top"><input id="titleRow" onClick="selectOne(this)" name="title<?=$row_cat['id']?>" type="radio"  <?=$chek0?>></td>
      <td align="center" valign="top"><input name="list<?=$row_cat['id']?>" type="checkbox" id="list<?=$row_cat['id']?>" <?=$chek?>></td>
      <td align="center" valign="top"><input name="search<?=$row_cat['id']?>" type="checkbox" id="search<?=$row_cat['id']?>" <?=$chek1?>></td>
...
?><div style="background-color:#CCDCE6; width:100%; height:100%; margin:0px">
  <nobr>Поле: <select name="fromfield" id="fromfield" onChange="if(this.options[this.selectedIndex].value!=''){parent.document.getElementById('from_field').value=this.options[this.selectedIndex].value}else{parent.document.getElementById('from_field').value=''}">
	<option></option>
	<? do{ ?>
	<option value="<?=$row_list_field['field']?>" <?=(isset($_GET['from_field'])&&$_GET['from_field']==$row_list_field['field'])?"selected":""?>><?=$row_list_field['name']?></option>
...
?><div style="background-color:#CCDCE6; width:100%; height:100%; margin:0px">
  ...
  <nobr>Родительское поле: <select name="fromfield" id="fromfield" onChange="if(this.options[this.selectedIndex].value!=''){parent.document.getElementById('from_field').value=this.options[this.selectedIndex].value}else{parent.document.getElementById('from_field').value=''}">
	<option></option>
	<? do{ ?>
	<option value="<?=$row_list_field['field']?>" <?=(isset($_GET['from_field']) && $_GET['from_field']==$row_list_field['field'])?"selected":""?>><?=$row_list_field['name']?></option>
	<? }while($row_list_field=el_dbfetch($list_field)); ?>
...
$rop=el_dbfetch($op);
?>    
      <nobr>Дочерняя характеристика: <select name="childopt" id="childopt" onChange="if(this.options[this.selectedIndex].value!=''){parent.document.getElementById('child_opt').value=this.options[this.selectedIndex].value}else{parent.document.getElementById('child_opt').value=''}">
	<option></option>
	<? do{ ?>
	<option value="<?=$rop['field']?>" <?=(isset($_GET['child_opt']) && $_GET['child_opt']==$rop['field'])?"selected":""?>><?=$rop['name']?></option>
	<? }while($rop=el_dbfetch($op)); ?>
	</select></nodr><br>
      <nobr>Ключевое поле: <select name="childfield" id="childfield" onChange="if(this.options[this.selectedIndex].value!=''){parent.document.getElementById('child_field').value=this.options[this.selectedIndex].value}else{parent.document.getElementById('child_field').value=''}">
	<option></option>
	<? do{ ?>
	<option value="<?=$rch['field']?>" <?=(isset($_GET['child_field']) && $_GET['child_field']==$rch['field'])?"selected":""?>><?=$rch['name']?></option>
...
<form method="post">
  <tr>
    <td>Тип поля формы в административной части:<span style="background-color:#b1c5d2"><img src="/editor/img/help_button.gif" alt="Кликните для получения справки" width="12" height="12" style="cursor:pointer" onClick="test.TextPopup ('Здесь можно задать тип поля в форме добаыления позиции в каталог в административной части',pszFont,10,10,-1,-1)"></span> </td>
  ...
  <tr>
    ...
    <td><? if($row_cat['type']=="textarea" || $row_cat['type']=="full_html" || $row_cat['type']=="basic_html"){ ?><nobr>Ширина: 
      <input name="cols" type="text" id="cols<?=$row_cat['id']?>" value="<?=$row_cat['cols']?>" size="3" onBlur="MM_validateForm('cols<?=$row_cat['id']?>','','NisNum');return document.MM_returnValue"></nobr><br>
<nobr>Высота: <input name="rows" type="text" id="rows<?=$row_cat['id']?>" value="<?=$row_cat['rows']?>" size="3" onBlur="MM_validateForm('rows<?=$row_cat['id']?>','','NisNum');return document.MM_returnValue"></nodr><? }else{?><input name="size" type="text" id="size<?=$row_cat['id']?>" value="<?=$row_cat['size']?>" size="3" onBlur="MM_validateForm('size<?=$row_cat['id']?>','','NisNum');return document.MM_returnValue"></nobr><? }?></td>
  </tr>
  
  
  <? if($row_cat['type']=="option"||$row_cat['type']=="optionlist"){ ?>
  <tr>
    <td>Опции:<span style="background-color:#b1c5d2"><img src="/editor/img/help_button.gif" alt="Кликните для получения справки" width="12" height="12" style="cursor:pointer" onClick="test.TextPopup ('Здесь указываются пункты списка. Весь список будет показан в административной части, а на сайте только выбранные в административной части пункты.',pszFont,10,10,-1,-1)"></span> </td>
...
$row_list_db=el_dbfetch($list_db);
?> 
	<nobr>Каталог: <select name="listdb" id="listdb" onChange="field_frame.location.href('catalogs.php?catalog_id='+listdb.options[listdb.selectedIndex].value+'&viewfield&from_field=<?=$row_cat['from_field']?>')">
	<option></option>
	<? do{?>
		<option value="<?=$row_list_db['catalog_id']?>" <?=($row_list_db['catalog_id']==$row_cat['listdb'])?"selected":""?>><?=$row_list_db['name']?></option>
	<? }while($row_list_db=el_dbfetch($list_db)); ?>
	</select><input type="hidden" id="from_field" name="from_field"></nobr></td><td>
<script language="javascript">/*
...
<iframe frameborder="0" width="300" height="30" scrolling="no" id="field_frame" name="field_frame" hspace="0" marginheight="0" marginwidth="0" vspace="0" src="catalogs.php?catalog_id=<?=$row_cat['listdb']?>&viewfield&from_field=<?=$row_cat['from_field']?>"></iframe>
</td>
  </tr>
 <? } 
...
$row_list_db=el_dbfetch($list_db);
?> 
	<nobr>Родительский каталог: <select name="listdb" id="listdb" onChange="field_frame.location.href('catalogs.php?catalog_id='+listdb.options[listdb.selectedIndex].value+'&viewfield&depend&from_field=<?=$row_cat['from_field']?>')">
	<option></option>
	<? do{?>
		<option value="<?=$row_list_db['catalog_id']?>" <?=($row_list_db['catalog_id']==$row_cat['listdb'])?"selected":""?>><?=$row_list_db['name']?></option>
	<? }while($row_list_db=el_dbfetch($list_db)); ?>
	</select>
    <input type="hidden" id="from_field" name="from_field" value="<?=$row_cat['from_field']?>">
    <input type="hidden" id="child_opt" name="child_opt" value="<?=$row_cat['field']?>">
    <input type="hidden" id="child_field" name="child_field" value="<?=$row_cat['to_field']?>"></nobr></td><td>
<script language="javascript">/*
field_frame.location.href('catalogs.php?catalog_id='+listdb.options[listdb.selectedIndex].value+'&viewfield&from_field=<?=$row_cat['from_field']?>');*/
</script>
<iframe frameborder="0" width="300" height="30" scrolling="no" id="field_frame" name="field_frame" hspace="0" marginheight="0" marginwidth="0" vspace="0" src="catalogs.php?catalog_id=<?=$row_cat['listdb']?>&viewfield&depend&from_field=<?=$row_cat['from_field']?>"></iframe>
</td>
  </tr>
    <tr>
    <td>
...
$row_list_db=el_dbfetch($list_db);
?> 
	<nobr>Дочерний каталог: <select name="listdb_child" id="listdb_child" onChange="field_frame_child.location.href('catalogs.php?catalog_id='+listdb_child.options[listdb_child.selectedIndex].value+'&viewfield&depend&child_field=<?=$row_cat['to_field']?>&child_opt=<?=$row_cat['default_value']?>')">
	<option></option>
	<? do{?>
		<option value="<?=$row_list_db['catalog_id']?>" <?=($row_list_db['catalog_id']==$row_cat['options'])?"selected":""?>><?=$row_list_db['name']?></option>
	<? }while($row_list_db=el_dbfetch($list_db)); ?>
	</select></nobr></td><td>
<script language="javascript">/*
field_frame.location.href('catalogs.php?catalog_id='+listdb.options[listdb.selectedIndex].value+'&viewfield&from_field=<?=$row_cat['from_field']?>');*/
</script>
<iframe frameborder="0" width="300" height="60" scrolling="no" id="field_frame_child" name="field_frame_child" hspace="0" marginheight="0" marginwidth="0" vspace="0" src="catalogs.php?catalog_id=<?=$row_cat['options']?>&viewfield&depend&child_field=<?=$row_cat['to_field']?>&child_opt=<?=$row_cat['field']?>"></iframe>
</td>
  </tr>
...
$fflag=1;
	...
	if($totalRows_cats>0){echo "<script>alert('Каталог с таким названием уже есть! Выберите другое название.'); location.href='".$_SERVER['HTTP_REFERER']."'</script>"; $err=1;
	}
	
	$query_cats_check = "SELECT * FROM catalogs WHERE catalog_id='".$_POST['catalog_id']."'";
	$cats_check=el_dbselect($query_cats_check, 0, $cats_check);
	$totalRows_cats = mysqli_num_rows($cats_check);
	if($totalRows_cats>0){echo "<script>alert('Таблица с таким именем в базе данных уже есть! Выберите другое имя.'); location.href='".$_SERVER['HTTP_REFERER']."'</script>"; $err=1;
	}
	if($err!=1){
		...
		el_dbselect($query_catdata, 0, $res);//$dbconn) or die("Не могу создать таблицу $new_table! Ошибка: ".mysqli_error()()
		...
		if($fflag!=0){
		  $insertSQL = sprintf("INSERT INTO catalogs (name, catalog_id, lines_per_pages, cols_per_pages, modul_top, modul_bot, small_size, big_size, currency, shop, template, template_set, ftemplate, ftemplate_d) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
							   GetSQLValueString($_POST['name'], "text"),
							   GetSQLValueString($_POST['catalog_id'], "text"),
...
<form action="<?=$_SERVER['REQUEST_URI']?>&id" method="post" enctype="multipart/form-data" onSubmit="return fill(['name','catalog_id'],['Название каталога','Имя таблицы данных'])">
<table width="80%" border="0" align="center" cellpadding="5" cellspacing="0" class="el_tbl">
<caption>
Новый каталог 
</caption>
...
пикселей</td>
  ...
  */?>
<tr><td colspan="2"><center>Использовать шаблоны:<br>
<label for="tfiles"><input type="radio" id="tfiles" name="type_template" value="file" onClick="selectType('file')" <?=($_COOKIE['idshow']['cat_template']=='file')?'checked':''?>> В виде файлов</label>&nbsp;&nbsp;&nbsp;&nbsp;
<label for="tstring"><input type="radio" id="tstring" name="type_template" value="string" onClick="selectType('string')" <?=($_COOKIE['idshow']['cat_template']=='string')?'checked':''?>> В виде текста</label></center>
</td></tr>
...
(strlen($_FILES['template_df']['name'])>0)?$ftemplate_d=$_FILES['template_df']['name']:$ftemplate_d=$_POST['template_df'];
	if($fflag!=0){
				$updateSQL = sprintf("UPDATE catalogs SET name=%s, lines_per_pages=%s, cols_per_pages=%s, modul_top=%s, modul_bot=%s, small_size=%s, big_size=%s, currency=%s, shop=%s, template=%s, template_set=%s, ftemplate=%s, ftemplate_d=%s WHERE catalog_id=%s",
        		GetSQLValueString($_POST['name'], "text"),
				GetSQLValueString($_POST['lines_per_pages'], "int"),
				GetSQLValueString($_POST['cols_per_pages'], "int"),
				GetSQLValueString($_POST['modul_top'], "text"),
...
<form method="post" enctype="multipart/form-data" action="<?=$_SERVER['REQUEST_URI']?>" onSubmit="return fill(['name','catalog_id'],['Название каталога','Имя таблицы данных'])" name="editCat">
<table width="80%" border="0" align="center" cellpadding="5" cellspacing="0" class="el_tbl">
<caption>
...
<tr>
  ...
  <tr>
      <td width="42%">Имя таблицы <font color="#FF0000" size="2"> (по-английски)</font> <img src="/editor/img/help_button.gif" alt="Кликните для получения справки" width="12" height="12" style="cursor:pointer" onClick="test.TextPopup ('В этом поле указывается имя поля в базе данных английскими буквами. Например, name или height',pszFont,10,10,-1,-1)"></td>
      <td><input name="catalog_id" type="text" id="catalog_id" value="<?=$row_catalogs['catalog_id']?>" size="30"></td>
    </tr>
  <tr>
    <td>Количество позиций на странице: </td>
    <td><input name="lines_per_pages" type="text" id="lines_per_pages" value="<?=$row_catalogs['lines_per_pages']?>" size="5"></td>
  </tr>
  <tr>
    <td>Количество столбцов в списке: </td>
    <td><input name="cols_per_pages" type="text" id="cols_per_pages" value="<?=$row_catalogs['cols_per_pages']?>" size="5"></td>
  </tr>
  <tr>
 <? 
...
?>
   ...
   */?>
   <tr>
    <td>Ширина маленькой картинки:</td>
    <td><input name="small_size" type="text" id="small_size" value="<?=$row_catalogs['small_size']?>" size="5">
      пикселей</td>
  </tr>
  <tr>
    <td>Ширина большой картинки: </td>
...
<tr><td colspan="2"><center>Использовать шаблоны:<br>
<label for="tfile"><input type="radio" id="tfile" name="type_template" value="file" onClick="selectType('file')" <?=($_COOKIE['idshow']['cat_template']=='file')?'checked':''?>> В виде файлов</label>&nbsp;&nbsp;&nbsp;&nbsp;
<label for="tstring"><input type="radio" id="tstring" name="type_template" value="string" onClick="selectType('string')" <?=($_COOKIE['idshow']['cat_template']=='string')?'checked':''?>> В виде текста</label></center>
</td></tr>
<tr id="row1" style="display:<?=($_COOKIE['idshow']['cat_template']=='file')?'block':'none'?>">
    <td>Шаблон дизайна общего списка: </td>
...
//Шаблоны дизайна 
 function show_structure(){
	...
	<option></option>";
	$c=el_dbselect("SELECT name, catalog_id FROM catalogs", 0, $s);
	$ca=el_dbfetch($c);
	do{
	...
	if(isset($_POST['catalog_struc']) && strlen($_POST['catalog_struc'])>0){
		$s=el_dbselect("SELECT name, field FROM catalog_prop WHERE catalog_id='".$_POST['catalog_struc']."' ORDER BY sort", 0, $s);
		$st=el_dbfetch($s);
		do{
 ...
 if($_GET['mode']=="templates"){
	...
	if(!isset($_GET['id_temp']) && !isset($_GET['new'])){//Выводим список шаблонов
		...
		<div id="table_temp_child" style="display:'.$dispTable.'; margin-left:40px">'; 
		 ...
		 if(mysqli_num_rows($t)>0){
			 do{?>
			 <div class='row'><div id="left">№<?=$te['id']?> Шаблон &laquo;<?=$te['name']?>&raquo; [<?=$te['type']?>]</div><div id='right'><img border="0" src="/editor/img/menu_edit.gif" onClick="act1('edit', <?=$te['id']?>)">&nbsp;<img border="0" src="/editor/img/menu_delete.gif" onClick="act1('del', <?=$te['id']?>)"></div></div>
			 <?
	 ...
	 if(isset($_GET['id_temp'])){
		...
		<tr><td colspan="2"><? el_showalert('info', 
		...
		можно выделять и перетаскивать поля в соответсвующее поле в форме.
		 ')?></td></tr>
		<tr><td valign="top">Показать структуру каталога:</td><td valign="top"><? show_structure() ?></td></tr>
		</table>
		<table border="0" align="center" class="el_tbl">
			...
			<tr>
				...
				<td>
				<input name="name" id="name" type="text" value="<?=stripslashes(str_replace("'", '``', $et['name']))?>" size="40">
				</td>
			</tr>
			<tr>
				...
				<select name="type" id="type" onChange="type_form(this)">
				 <option value="Общий дизайн" <?=($et['type']=='Общий шаблон')?'selected':''?>>Общий дизайн</option>
				 <option value="Дизайн строки" <?=($et['type']=='Дизайн строки')?'selected':''?>>Дизайн строки</option>
				 </select>
			...
			<tr id="rowс" style="display:<?=($et['type']=='Дизайн строки')?'block':'none'?>">
				<td align="right">Цвет фона строк:</td><td>
				<table border="0" cellpadding="3" cellspacing="0">
				<tr>
				  <td>I цвет <input type="button" name="Button" value="Выбрать" class="but" onClick="callColorDlg('1bgc', 'bgc1')">
				  <input type="text" size="7" id="1bgc" name="1bgc" value="<?=$et['1bgc']?>"></td>
				  <td style="background-color:#<?=strtoupper($et['1bgc'])?>" id="bgc1"><img src="/editor/img/spacer.gif" width="20" height="20"></td>
				  <td>II цвет <input type="button" name="Button" value="Выбрать" class="but" onClick="callColorDlg('2bgc', 'bgc2')">
				   <input type="text" size="7" id="2bgc" name="2bgc" value="<?=$et['2bgc']?>"></td>
				  <td style="background-color:#<?=strtoupper($et['2bgc'])?>" id="bgc2"><img src="/editor/img/spacer.gif" width="20" height="20"></td>
				</tr>
			  </table>
				</td>
			</tr>
			...
			<tr id="row0" style="display:<?=($et['type']=='Общий дизайн')?'block':'none'?>">
				...
				<td>
				<textarea name="top_list" rows="10" cols="50"><?=stripslashes($et['top_list'])?></textarea><br>      
			...
			<tr id="row1" style="display:<?=($et['type']=='Общий дизайн')?'block':'none'?>">
				...
				<td>
				<textarea name="bottom_list" rows="10" cols="50"><?=stripslashes($et['bottom_list'])?></textarea><br>      
			...
			<tr id="row2" style="display:<?=($et['type']=='Общий дизайн')?'block':'none'?>">
				...
				<td>
				<textarea name="top_detail" rows="10" cols="50"><?=stripslashes($et['top_detail'])?></textarea><br>      
			...
			<tr id="row3" style="display:<?=($et['type']=='Общий дизайн')?'block':'none'?>">
				...
				<td>
				<textarea name="bottom_detail" rows="10" cols="50"><?=stripslashes($et['bottom_detail'])?></textarea><br>      
			...
			<tr>
				<td colspan="2" align="center">
				<input type="submit" value="Сохранить" onClick="act_mode.value=''" class="but">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value="Сохранить как новый" onClick="new_template()" class="but">
	...
	if(isset($_GET['new'])){
		...
		можно выделять и перетаскивать поля в соответсвующее поле в форме.
		 ')?></td></tr>
		<tr><td valign="top">Показать структуру каталога:</td><td valign="top"><? show_structure() ?></td></tr></table>
		<table border="0" align="center" class="el_tbl">
			<form method="post" name="temp_form" onSubmit="return check_type()">
			<tr>
				<td align="right">Название шаблона:</td>
