...
el_reg_work($work_mode, $_SESSION['login'], $_GET['cat']);
...
<head>
    ...
    <script language="javascript">
        <!--
        function check_el(item_name, id) {
            var OK = confirm('Вы действительно хотите удалить поле "' + item_name + '" ?');
            if (OK) {
                document.getElementById('cat_mode_edit1') .value='del';
...
//var ptbl = 
        ...
        function MM_validateForm() {
            ...
            if (errors) alert('Ошибка заполнения формы:\n' + errors);
...
<p>
    ...
    | <a href="catalogs.php?mode=new">Создать каталог</a>
    | <a href="catalogs.php?mode=list">Список каталогов</a>
    | <a href="catalogs.php?mode=templates">Шаблоны внешнего вида</a>
</p>
<h4 align="center">Список характеристик каталогов</h4>
...
$cats = el_dbselect("SELECT * FROM catalogs", 0, $res);
$row_cats = el_dbfetch($cats);

if ($_POST['cat_mode_edit1'] == "del") {
    $deleteSQL = sprintf("DELETE FROM catalog_options WHERE id=%s",
...
if ($_POST['cat_mode_edit1'] == "save") {
    ...
    $catsave = el_dbselect("SELECT * FROM catalog_options", 0, $res);
    $row_catsave = el_dbfetch($catsave);
    do {
        $updateSQL = sprintf("UPDATE catalog_options SET name=%s, sort=%s WHERE id=%s",
...
if ((isset($_POST['new_element'])) && ($_POST['new_element'] == "1")) {
    $insertSQL = sprintf("INSERT INTO catalog_options (name, type, size, cols, rows, sort, options, listdb, from_field) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
$cat2 = el_dbselect("SELECT MAX(sort) FROM catalog_options", 0, $res);
if ($cat2) $row_cat2 = el_dbfetch($cat2);

$cat = el_dbselect("SELECT * FROM catalog_options ORDER BY sort", 0, $cat);
        ...
        <? do { ?>
            <tr>
                <td valign="top" id="string<?= $row_cat['id'] ?>">
                    ...
                    <input name="name<?= $row_cat['id'] ?>" type="text" id="name<?= $row_cat['id'] ?>"
                           value="<?= htmlspecialchars($row_cat['name']) ?>" size="30">
                </td>
                ...
                <td align="center"><i title="Редактировать" class="material-icons"
                                      onClick="MM_openBrWindow('catalogs.php?mode=editfield&field=<?= $row_cat['id'] ?>','edit','scrollbars=no,resizable=yes,width=750,height=310')">edit</i>
                </td>
                <td align="center"><i class="material-icons" title="Удалить"
                                      onClick="check_el('<?= htmlspecialchars($row_cat['name']) ?>', <?= $row_cat['id'] ?>)">delete_forever</i>
                </td>
            </tr>
...
<form method="post" action="<?= $_SERVER['REQUEST_URI'] ?>#new" name="newform">
    <table width="500" border="0" align="center" cellpadding="5" cellspacing="0" class="el_tbl">
        <caption>
            Новое поле
        </caption>
        <tr>
            <td width="42%" align="right">Название <img src="/editor/img/help_button.gif"
                                                        alt="Кликните для получения справки" width="12" height="12"
                                                        style="cursor:pointer"
                                                        onClick="test.TextPopup ('В этом поле указывается название поля для отображения в каталоге. Например, Имя или Высота',pszFont,10,10,-1,-1)">
            </td>
        ...
        <tr>
            <td align="right">Тип <img src="/editor/img/help_button.gif" alt="Кликните для получения справки" width="12"
                                       height="12" style="cursor:pointer"
                                       onClick="test.TextPopup ('Выберите тип будущего поля. Описание типов приведено в документации.',pszFont,10,10,-1,-1)">
            </td>
            <td>
        ...
        <tr>
            <td align="right">Размер <img src="/editor/img/help_button.gif" alt="Кликните для получения справки"
                                          width="12" height="12" style="cursor:pointer"
                                          onClick="test.TextPopup ('Задайте размер будущего поля. Этот параметр важен только для административной части.',pszFont,10,10,-1,-1)">
            </td>
            <td>
                <div id="sizefield" style="display:block"><input name="size" type="text" id="size" size="3"></div>
                <div id="area" style="display:none">
                    <nobr>Ширина: <input name="cols" type="text" id="cols" value="40" size="3"></nobr>
                    <br>
                    <nobr>Высота: <input name="rows" type="text" id="rows" value="5" size="3"></nobr>
                </div>
            </td>
        </tr>
        <tr>
            <td align="right">Номер <img src="/editor/img/help_button.gif" alt="Кликните для получения справки"
                                         width="12" height="12" style="cursor:pointer"
                                         onClick="test.TextPopup ('Задайте порядковый номер будущего поля. Этот номер определяет положение поля в описании товара или услуги.',pszFont,10,10,-1,-1)">
            </td>
