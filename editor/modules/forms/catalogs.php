...
<head>
    ...
    <style type="text/css">
        ...
        td#sampleField textarea, td#sampleField select {
            width: 150px
        }
...
<center>
    ...
    <p><a href="params.php<?= (!empty($_GET['cat'])) ? '?cat=' . $_GET['cat'] : '' ?>">Список полей</a>
        ...
        if (!empty($_GET['id'])) { ?>
            |
            <a href="/editor/modules/forms/edit_form.php?id=<?= $_GET['id'] ?><?= (!empty($_GET['cat'])) ? '&cat=' . $_GET['cat'] : '' ?>">Редактировать
                настройки формы &raquo;</a>
        <? } ?>
    </p>
</center>
<?
...
if (($_GET['mode'] == "list") || (!isset($_GET['mode'])) && (!isset($_GET['cid'])) && !isset($_GET['new_id']) && (!isset($_GET['id'])) && (!isset($_GET['viewfield']))) {//Показ списка каталогов
    ...
    if ($totalRows_cats < 1) {
        echo "<h4 align='center'>Нет ни одной формы</h4>";
    } else {
        ?>
            ...
            <? do { ?>
                <tr id="string<?= $row_cats['id']; ?>">
                    <form method="post" action="<?= $_SERVER['REQUEST_URI'] ?>">
                        <td width="50%" style="color:#003399"
                            onMouseOver='document.getElementById("string<?= $row_cats['id']; ?>").style.backgroundColor="#DEE7EF"'
                            onMouseOut='document.getElementById("string<?= $row_cats['id']; ?>").style.backgroundColor="#CCDCE6"'><input name="name" type="text"
                                                                                                                                         value="<?= $row_cats['name'] ?>"
                                                                                                                                         size="40"></td>
                        <td>
                            ...
                            <a href="edit_form.php?id=<?= $row_cats['id'] ?>"><img src="/editor/img/leftmenu_tools.gif" alt="Настройки формы" border="0"></a>
                        </td>
                        <td><a href="catalogs.php?id=<?= $row_cats['id'] ?>"><img src="/editor/img/menu_edit.gif" alt="Редактировать структуру" width="23"
                                                                                  height="17" border="0"></a></td>
                        <td><input name="submit_save" type="image" id="submit_save2" src="/editor/img/menu_save.gif" alt="Сохранить изменения"
                                   onClick="cat_mode.value='save';">
                        </td>
                        <td><input name="submit_del" type="image" id="submit_del" src="/editor/img/menu_delete.gif" alt="Удалить форму"
...
if ((isset($_GET['id']) || isset($_GET['edit_cat']) || isset($_GET['new_id'])) && $_GET['mode'] != "new") {
    ...
    if (isset($_GET['new_id']) && $row_cats['from_catalog'] == '1') {
        ...
        do {
            ...
            $insertSQL = sprintf("INSERT INTO form_prop (name, type, size, cols, rows, sort, title, inform, required, catalog_id, field, options, listdb, from_field) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            ...
            <table border="0" align="center" cellpadding="5" cellspacing="0" class="el_tbl">
                ...
                <tr>
                    ...
                    <td align="center" style="background-color:#b1c5d2">Значение<br>
                        по умолчанию <img src="/editor/img/help_button.gif" alt="Кликните для получения справки" width="12" height="12" style="cursor:pointer"
                                          onClick="test.TextPopup ('Этот параметр показывает, какое значение будет задано полю по умолчанию',pszFont,10,10,-1,-1)">
                    </td>
                    <td align="center" style="background-color:#b1c5d2">Заголовок <img src="/editor/img/help_button.gif" alt="Кликните для получения справки"
                    ...
                    <td align="center" style="background-color:#b1c5d2">В форме <img src="/editor/img/help_button.gif" alt="Кликните для получения справки"
                                                                                     width="12" height="12" style="cursor:pointer"
                    ...
                    <td align="center" style="background-color:#b1c5d2">Обязательное<br>
                        поле<img src="/editor/img/help_button.gif" alt="Кликните для получения справки" width="12" height="12" style="cursor:pointer"
                                 onClick="test.TextPopup ('Этот параметр показывает, будет ли это поле обязательным к заполнению',pszFont,10,10,-1,-1)"></td>
                    <td align="center" style="background-color:#b1c5d2">Номер <img src="/editor/img/help_button.gif" alt="Кликните для получения справки"
                                                                                   width="12" height="12" style="cursor:pointer"
                ...
                <? do {
                    ...
                    <tr onMouseOver="line_over('<?= $row_cat['id'] ?>')" onMouseOut="line_out('<?= $row_cat['id'] ?>')">
                        <td valign="top" id="string<?= $row_cat['id'] ?>"><?
                            ...
                            ?>
                            <img src="/editor/img/spacer.gif" name="img<?= $row_cat['id'] ?>" width="19" height="17" align="absmiddle"
                                 id="img<?= $row_cat['id'] ?>">
                            <nobr>
                        ...
                        <td valign="top"
                            id="sampleField"><?= el_viewField($row_cat['type'], $row_cat, 'defval' . $row_cat['id'], $row_cat['default_value']) ?></td>
                        <td align="center" valign="top"><input id="titleRow" onClick="selectOne(this)" name="title<?= $row_cat['id'] ?>"
                                                               type="radio" <?= $chek0 ?>></td>
                        ...
                        <td align="right" valign="top"><input name="sort<?= $row_cat['id'] ?>" type="text" id="sort<?= $row_cat['id'] ?>"
                                                              onBlur="MM_validateForm('sort<?= $row_cat['id'] ?>','','NisNum');return document.MM_returnValue"
                                                              value="<?= $row_cat['sort'] ?>" size="3"></td>
                        <td align="center"><input name="submit_del2" type="image" id="submit_del2" src="/editor/img/menu_delete.gif" alt="Удалить"
                                                  onClick="delParam(<?= $row_cat['id'] ?>, '<?= $row_cat['name'] ?>', <?= $row_cat['field'] ?>)"></td>
                    </tr>
			...
			<a href="params.php">Список полей</a><br>и создайте набор полей.</span>';
                    ...
                    <div id="fromdb" style="display:none"><br>
                        ...
                        <div style="display:none" id="field_div">
                            <iframe frameborder="0" width="300" height="30" scrolling="no" id="field_frame" name="field_frame" hspace="0" marginheight="0"
                                    marginwidth="0" vspace="0"></iframe>
                        </div>
                    </div>
                </td>
            </tr>
...
if (isset($_GET['catalog_id']) && isset($_GET['viewfield'])) {
    ...
    <div style="background-color:#CCDCE6; width:100%; height:100%; margin:0px">
        <nobr>Поле:
            <select name="fromfield" id="fromfield"
                    onChange="if(fromfield.options[fromfield.selectedIndex].value!=''){parent.document.getElementById('from_field').value=fromfield.options[fromfield.selectedIndex].value}else{parent.document.getElementById('from_field').value=''}">
                <option></option>
                <? do { ?>
...
if ($_GET['mode'] == "editfield") {
    if (isset($_POST['update'])) {
        ...
        $id_cat = $_GET['catalog'];
        $updateSQL = sprintf("UPDATE form_options SET type=%s, size=%s, cols=%s, rows=%s, options=%s, listdb=%s, from_field=%s WHERE id=%s",
        ...
        $updateSQL = sprintf("UPDATE form_prop SET type=%s, size=%s, cols=%s, rows=%s, options=%s, listdb=%s, from_field=%s WHERE option_id=%s",
            GetSQLValueString($_POST['type'], "text"),
            GetSQLValueString($_POST['size'], "int"),
            GetSQLValueString($_POST['cols'], "int"),
            GetSQLValueString($_POST['rows'], "int"),
    ...
    <table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="el_tbl">
        ...
        <form method="post">
            <tr>
                <td>Тип поля формы:<span style="background-color:#b1c5d2"><img src="/editor/img/help_button.gif" alt="Кликните для получения справки" width="12"
                                                                               height="12" style="cursor:pointer"
                                                                               onClick="test.TextPopup ('Здесь можно задать тип поля в форме',pszFont,10,10,-1,-1)"></span>
                </td>
                <td><select name="type" id="type">
                    ...
                    <br>
                    <span style="color:red">Внимание!</span> Смена строкового типа на числовой приведет к потере имеющихся данных.
                </td>
            </tr>
            <tr>
                <td>Размер поля формы:<span style="background-color:#b1c5d2"><img src="/editor/img/help_button.gif" alt="Кликните для получения справки"
                                                                                  width="12" height="12" style="cursor:pointer"
                                                                                  onClick="test.TextPopup ('Этот параметр указывает размер поля формы',pszFont,10,10,-1,-1)"></span>
                </td>
                <td><? if ($row_cat['type'] == "textarea") { ?>
                        <nobr>Ширина:
                            <input name="cols" type="text" id="cols<?= $row_cat['id'] ?>" value="<?= $row_cat['cols'] ?>" size="3"
                                   onBlur="MM_validateForm('cols<?= $row_cat['id'] ?>','','NisNum');return document.MM_returnValue">
                        </nobr><br>
                        <nobr>Высота:
                        <input name="rows" type="text" id="rows<?= $row_cat['id'] ?>" value="<?= $row_cat['rows'] ?>" size="3"
                               onBlur="MM_validateForm('rows<?= $row_cat['id'] ?>','','NisNum');return document.MM_returnValue">
                        </nodr>
                    <? } else { ?>
                        <input name="size" type="text" id="size<?= $row_cat['id'] ?>" value="<?= $row_cat['size'] ?>" size="3"
                               onBlur="MM_validateForm('size<?= $row_cat['id'] ?>','','NisNum');return document.MM_returnValue">
                        </nobr>
                    <? } ?></td>
            </tr>
            <? if ($row_cat['type'] == "option" || $row_cat['type'] == "optionlist") { ?>
                <tr>
                    <td valign=top>Опции:<span style="background-color:#b1c5d2"><img src="/editor/img/help_button.gif" alt="Кликните для получения справки"
                                                                                     width="12" height="12" style="cursor:pointer"
...
if ($_GET['mode'] == "new") {
    ...
    if ($_POST['formsUpdate'] == "new") {
        ...
        if ($err == 0) {
            ...
            el_dbselect("INSERT INTO forms (name, title, cat, method, action, target, sorce_table, catalog_id, from_catalog, type, ajax, email, email_charset, email_type, answer, protect) VALUE (
			" . GetSQLValueString($_POST['name'], 'text') . ",
    ...
    <form action="<?= $_SERVER['REQUEST_URI'] ?>&id" method="post" enctype="multipart/form-data"
          onSubmit="return fill(['name','tbl_name'],['Название формы','Имя таблицы данных'])">
        <table border="0" cellspacing="0" cellpadding="4" align="center" class="el_tbl">
            ...
            <tr>
                <td>Имя таблицы данных <font color="#FF0000" size="1">(по-английски)</font> <img src="/editor/img/help_button.gif"
                                                                                                 alt="Кликните для получения справки" width="12" height="12"
