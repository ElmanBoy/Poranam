...
if(mysqli_num_rows($template)>0){
	...
	do { 
		$c++;
		($c==mysqli_num_rows($template))?$rowEnd='_end':$rowEnd='';
	?>
...
<?php } while ($row_template = el_dbfetch($template)); 
}else{
...
<?/*div onClick="showhideDiv('PHP_temp')" id="PHP_temp" class="row_<?=$dispPHP?>"><h5>Готовые шаблоны</h5></div*/?>
		<div id="PHP_temp_child" style="margin-left:40px; margin-top:-10px">
<?php 
$template=el_dbselect("SELECT * FROM template WHERE `master`!=1", 0, $template);
$row_template = el_dbfetch($template);
if(mysqli_num_rows($template)>0){
	...
	do { 
		...
		<div id="right">
            <?/*i class="material-icons" onClick="MM_goToURL('self','/editor/e_modules/template_editor.php?id=<?=$row_template['id']?>');return document.MM_returnValue" title="Редактировать шаблон">edit</i>&nbsp;*/?>
            <i class="material-icons" onClick="act1('save', <?=$row_template['id']?>)" title="Сохранить изменения">save</i>&nbsp;
            <i class="material-icons" onClick="MM_openBrWindow('/editor/e_modules/template_view.php?id=<?php echo $row_template['id']; ?>','view','status=yes,scrollbars=yes,resizable=yes','1024','768','true')" title="Просмотреть шаблон">screen_share</i>&nbsp;
            <i class="material-icons" onClick="act1('del', <?=$row_template['id']?>)" title="Удалить шаблон">delete_forever</i>
		</div></div>
<?php } while ($row_template = el_dbfetch($template)); 
}else{
...
<table width="50%"  border="0" align="center" cellpadding="5" cellspacing="0" class="el_tbl">
  ...
  <tr>
      <td colspan="2" align="center"><b>Добавить новый шаблон</b></td>
  </tr>
    <tr>
    ...
    <tr>
      <td align="right">Файл шаблона: </td>
    ...
    <tr>
      ...
      <input name="master" type="hidden" value="0">
