...
if (!empty($_POST['login']) && isset($_POST['pass'])) {
	if (@mysqli_connect(DBHOST, $_POST['login'], $_POST['pass'], DBNAMES)){
...
elseif (!empty($_COOKIE['sxd'])) {
	...
	if (@mysqli_connect(DBHOST, $user[1], $user[2], DBNAMES)){
		$auth = 1;
	}
	else{
		$error = '#' . mysqli_errno() . ': ' . mysqli_error();
...
class dumper {
    ...
    public function setLink($dbLink){
        $this->dbconn = $dbLink;
    }

    function dumper($dbconn) {
		...
		preg_match("/^(\d+)\.(\d+)\.(\d+)/", mysqli_get_server_info($dbconn), $m);
	...
	function backup() {
		...
		$result = mysqli_query($this->dbconn, "SHOW TABLE STATUS");
		$tabinfo = array();
		$tab_charset = array();
		$tab_type = array();
		$tabinfo[0] = 0;
		...
		$result = mysqli_query($this->dbconn, "SET SQL_QUOTE_SHOW_CREATE = 1");
		// Кодировка соединения по умолчанию
		if ($this->mysql_version > 40101 && CHARSET != 'auto') {
			mysqli_query($this->dbconn, "SET NAMES '" . CHARSET . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>" . mysqli_error());
        ...
        foreach ($tables AS $table){
			...
			if ($this->mysql_version > 40101 && $tab_charset[$table] != $last_charset) {
				if (CHARSET == 'auto') {
					mysqli_query($this->dbconn, "SET NAMES '" . $tab_charset[$table] . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>" . mysqli_error()());
        	...
        	// Создание таблицы
			$result = mysqli_query($this->dbconn, "SHOW CREATE TABLE `{$table}`");
        	$tab = el_dbfetch($result);
            ...
            $NumericColumn = array();
            $result = mysqli_query($this->dbconn, "SHOW COLUMNS FROM `{$table}`");
            $field = 0;
			...
			$this->fn_write($fp, "INSERT INTO `{$table}` VALUES");
                while(($result = mysqli_query($this->dbconn, "SELECT * FROM `{$table}` LIMIT {$from}, {$limit}")) && ($total = mysqli_num_rows($result))){
            		while($row = mysqli_fetch_row($result)) {
						...
						for($k = 0; $k < $fields; $k++){
                    		if ($NumericColumn[$k])
                    			...
                    			$row[$k] = isset($row[$k]) ? "'" . addslashes($row[$k]) . "'" : "NULL";
                    	}
	...
	function restore(){
		...
		if ($this->mysql_version > 40101 && (CHARSET != 'auto' || $this->forced_charset)) { // Кодировка по умолчанию, если в дампе не указана кодировка
			mysqli_query($this->dbconn, E_USER_ERROR, "SET NAMES '" . $this->restore_charset . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>" . mysqli_error());
			...
			if ($sql) {
			    if (preg_match("/;$/", $str)) {
						...
						elseif (preg_match("/CREATE TABLE/i", $sql)){
							...
							if (preg_match("/(CHARACTER SET|CHARSET)[=\s]+(\w+)/i", $sql, $charset)) {
								if (!$this->forced_charset && $charset[2] != $last_charset) {
									if (CHARSET == 'auto') {
										mysqli_query($this->dbconn, E_USER_ERROR, "SET NAMES '" . $charset[2] . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>{$sql}<BR>" . mysqli_error()());
							...
							elseif(CHARSET == 'auto'){ // Вставляем кодировку для таблиц, если она не указана и установлена auto кодировка
								...
								if ($this->restore_charset != $last_charset) {
									mysqli_query($this->dbconn, E_USER_ERROR, "SET NAMES '" . $this->restore_charset . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>{$sql}<BR>" . mysqli_error());
					...
					elseif($this->mysql_version > 40101 && empty($last_charset)) { // Устанавливаем кодировку на случай если отсутствует CREATE TABLE
						mysqli_query($this->dbconn, E_USER_ERROR, "SET $this->restore_charset '" . $this->restore_charset . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>{$sql}<BR>" . mysqli_error()());
						echo tpl_l("Установлена кодировка соединения `" . $this->restore_charset . "`.", C_WARNING);
						$last_charset = $this->restore_charset;
					}
            		$insert = '';
    			...
    			if ($execute) {
            		$q++;
            		mysqli_query($this->dbconn, E_USER_ERROR, $sql) or trigger_error ("Неправильный запрос.<BR>" . mysqli_error()());
	...
	function db_select(){
		if (DBNAMES != '') {
            ...
            foreach($items AS $item){
                if (mysqli_select_db($this->dbconn, $item)) {
                    $tables = mysqli_query($this->dbconn, "SHOW TABLES");
                    if ($tables) {
    		...
    		$result = mysqli_query($this->dbconn, "SHOW DATABASES");
    		$dbs = array();
