...
(function()
{
	...
	var names = [ 'play', 'loop', 'menu', 'quality', 'scale', 'salign', 'wmode', 'bgcolor', 'base', 'flashvars', 'allowScriptAccess',
		'allowFullScreen', 'movie', 'id', 'name' ];
	for ( var i = 0 ; i < names.length ; i++ )
		attributesMap[ names[i] ] = [ { type : ATTRTYPE_EMBED, name : names[i] }, { type : ATTRTYPE_PARAM, name : names[i] } ];
	names = [ 'allowFullScreen', 'play', 'loop', 'menu', 'allowScriptAccess' ];
	for ( i = 0 ; i < names.length ; i++ )
		attributesMap[ names[i] ][0]['default'] = attributesMap[ names[i] ][1]['default'] = true;
	
	attributesMap['allowScriptAccess'][0]=attributesMap['allowScriptAccess'][1]='true';
	attributesMap['src'][0]=attributesMap['src'][1]='/player.swf';
	...
	CKEDITOR.dialog.add( 'flashVideo', function( editor )
	{
		...
		return {
			title : 'Свойства видео-плеера',//editor.lang.flash.title,
			minWidth : 500,
			minHeight : 310,
			onShow : function()
			{
			...
			onOk : function()
			{
				...
				if ( !this.fakeImage )
				{
					if ( makeObjectTag )
					{
						...
						this.setValueOf( 'properties', 'flashvars', 'file='+srcVideo );
						var attributes = {
							classid : 'clsid:d27cdb6e-ae6d-11cf-96b8-444553540000',
							codebase : 'http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0',
							flashvars : 'file='+srcVideo,
							id:'player1',
							name:'player1'
						};
									...
									[
										{
											id : 'src',
											type : 'text',
											label : 'URL видео-файла',//editor.lang.common.url,
											required : true,
											...
											onLoad : function()
											{
												...
												updatePreview = function( src ){
													...
													srcVideo=CKEDITOR.tools.htmlEncode( previewPreloader.getAttribute( 'src' ) );
													dialog.preview.setHtml( '<embed height="100%" width="100%" src="/player.swf" flashvars="file='
														+ srcVideo
