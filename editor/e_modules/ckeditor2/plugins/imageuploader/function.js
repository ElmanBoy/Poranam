...
function showImage(imgSrc, imgStyle) {
    var typeArr = imgSrc.split(".");
    var typeExt = typeArr[typeArr.length - 1].toLowerCase();
    $(".preview").css("display", "none");
    if(typeExt == 'mp4' || typeExt == 'avi' || typeExt == 'mpeg' || typeExt == 'mpg' || typeExt == 'ogv'){
        $("#imageFSvideo").css("display", "block").attr('src', imgSrc);
    }else{
        $("#imageFSimg").css({"display": "block", "max-width": imgStyle + 'px'}).attr('src', imgSrc);
    }

    $("#imageFullSreen").show();
    $("#background").slideDown(250, "swing");

    $("#imgActionUse").attr("onclick", "parent.useImage('" + imgSrc + "')");
...
function showEditBar(imgSrc, imgStyle, imgID, imgName) {
    ...
    var imgID = imgID;
    var imgName = imgName;
    var deletePath = imgSrc.replace(location.origin, '').replace('//', '');
    ...
    $("#editbarDelete").attr("onclick","deleteImg('" + deletePath + "', '" + imgID + "');");
    $("#editbarUse").attr("onclick","parent.useImage('" + imgSrc + "')");
