...
//Наполняем каталог полями
  ...
  if(isset($_GET['id']) && strlen($_GET['id'])>0){$id_cat=$_GET['id'];}else{$id_cat=$_GET['new_id'];}
	...
	if(isset($_POST['new_element']) && $_POST['new_element']=="1"){
		$insertSQL = sprintf("INSERT INTO form_options (name, type, size, cols, rows, sort, options, listdb, from_field, revalid) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
<div id="area" style="display:none"> 
    ...
    <tr>
      <td align="right">Заполнять дважды</td>
      <td><input name="revalid" type="checkbox" id="revalid" value="1"></td>
    </tr>
    <tr>
