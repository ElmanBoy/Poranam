...
class UploadHandler {
    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        if ($this->toBytes(ini_get('post_max_size')) < $this->sizeLimit ||
            ...
            return array('error'=>"Серверная ошибка. Параметр post_max_size и upload_max_filesize меньше ".$neededRequestSize);
        }

        if ($this->isInaccessible($uploadDirectory)){
            return array('error' => "Серверная ошибка. Права доступа не позволяют записывать файлы в целевую папку.");
        }
        ...
        if(!isset($type)) {
            ...
            return array('error' => "Серверная ошибка. Not a multipart request. Please set forceMultipart to default value (true).");
        }
        ...
        # non-chunked upload
            ...
            $target = join(DIRECTORY_SEPARATOR, array($uploadDirectory, $uuid, $name)); //echo $target;
			...
			if ($target){
                ...
                if (move_uploaded_file($file['tmp_name'], $target)){
                	...
                	$uniq_id=md5($file['tmp_name'] + rand()*100000);
					$file_id = '/images/small/small_'.$uniq_id.'.'.$fileExt;
					$small_file_id = 'small_'.$uniq_id.'.'.$fileExt;
					$big_file_id='big_'.$uniq_id.'.'.$fileExt;

					//Добавление новой картинки
					$err=$both=0;
					$errStr='';
					$_POST['swidth']=220;//(strlen($_POST['swidth'])>0)?intval($_POST['swidth']):$site_property['gallerySWidth'.$cat];
					$_POST['sheight']=220;//(strlen($_POST['sheight'])>0)?intval($_POST['sheight']):$site_property['gallerySHeight'.$cat];
					...
					chmod($target.$big_file_id, 0777);

					if(el_resize_images($target, $small_file_id, $_POST['swidth'], $_POST['sheight'], 'small/')){
					...
					if(el_resize_images($target, $big_file_id, $_POST['bwidth'], $_POST['bheight'], 'gallery/')){
					...
					$_SESSION[$uuid] = $file_id;
                	return array('success'=> true, "uuid" => $uuid, "path" => $file_id);
                }
            }
    ...
    public function handleDelete($uploadDirectory, $name=null)
    {
        ...
        if (is_dir($target)){
			...
			unlink($_SERVER['DOCUMENT_ROOT'].$_SESSION[$uuid]);
			unlink($_SERVER['DOCUMENT_ROOT'].str_replace('/images/small/small_', '/images/gallery/big_' , $_SESSION[$uuid]));
            return array("success" => true, "uuid" => $uuid, "name" => $_SESSION[$uuid]);
        } else {
            return array("success" => false,
