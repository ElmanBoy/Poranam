...
function loadImages($path = '') {
    ...
    $useruploadpath = ($path != '') ? $path.'/' : $useruploadpath;
    $useruploadfolder = ($path != '') ? str_replace($_SERVER['DOCUMENT_ROOT'], '', $path) : $useruploadfolder;
    ...
    if(file_exists($useruploadpath)){
        ...
        $dir = $useruploadpath;
        $files = glob("$dir*.{jpg,jpe,jpeg,png,gif,ico,svg,avi,mpeg,mpg,mp4,ogv,JPG,JPEG,JPE,PNG,GIF,ICO,SVG,AVI,MPEG,MPG,MP4,OGV}", GLOB_BRACE);
        ...
        for($i=count($files)-1; $i >= 0; $i--):
            ...
            $image_url = str_replace('//images', '/images', $site.$useruploadfolder."/".$image_basename);
