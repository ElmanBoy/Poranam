...
//$_SESSION['site_id'] = intval($_GET['site_id']);
...
/*$c = el_dbselect("SELECT * FROM cat WHERE name = 'Архив новостей' AND cat_type=2", 0, $c, 'result', true);
$rc = el_dbfetch($c);
do{
    //$pa = el_dbselect("SELECT id FROM cat WHERE name='О центре' AND site_id=".intval($rc['site_id']), 0, $pa, 'row', true);
    $u = el_dbselect("UPDATE cat SET parent=421, cat_type=1 WHERE id=".intval($rc['id']), 0, $u, 'result', true);
...
$site = el_dbselect("SELECT * FROM sites WHERE id=" . $_SESSION['site_id'], 0, $sites, 'row', true);
?>
...
<body>
<h5>Управление разделами сайта &laquo;<?= $site['short_name'] ?>&raquo;</h5>
<br>
<? if ($_SESSION['user_level'] < 2) { ?>
    <button title="Добавить новый раздел в меню"
            onClick="MM_openBrWindow('newcategory.php?parentid=0&site_id=<?= $_SESSION['site_id'] ?>&cat_type=1','newcat','scrollbars=no,resizable=yes','720',
                    '500','true')">
        <i class="material-icons">add_circle</i> Создать новый раздел
    </button>
<? } ?>
<input id="saveSort" value="Сохранить изменения" onclick="document.edit.submit()" class="but" style="display:none; margin-left: 20px;">
<br>
<br>
<?
...
{//Child Items
    static $level;
    global $userLevel;
    $querymenuchild = "SELECT * FROM cat WHERE parent='$parent' AND site_id=" . intval($_SESSION['site_id']) . " AND cat_type < 2 ORDER BY sort ASC";
    ...
    if ($idchild) {//if item is exist...
        ...
        do {
            ...
            if (in_array($userLevel, $araccess) || intval($userLevel) < 4) {
                ?>
                <li class="child" id="tr<?php echo $row_menuchild['id']; ?>">
                    ...
                    <div class="parent4">
                        <table border="0" cellpadding="0" cellspacing="0">
                            <tr>
                                <td align="center" class="lbr"><a href="editor.php?cat=<?php echo $row_menuchild['cat_id']; ?>" target="Main"
                                ...
                                if ($_SESSION['user_level'] < 1) {
                                    ...
                                    <td align="center" class="lbr">
                                        <a href="javascript:void(0)" title="Добавить подраздел">
                                            <i onClick="MM_openBrWindow('newcategory.php?parentid=<?= $row_menuchild['cat_id']
                                            ?>&site_id=<?= $_SESSION['site_id'] ?>&cat_type=1','newcat',
...
{//Parent items, first level only
    ...
    $querymenutree = "SELECT * FROM cat WHERE parent=0 AND site_id=" . intval($_SESSION['site_id']) . " AND cat_type < 2 ORDER BY sort ASC";
            ...
            if (in_array($userLevel, $araccess) || intval($userLevel) < 4) {
                ...
                <li class="parent" id="tr<?= $row_menutree['id']; ?>">
                    ...
                    <div class="parent1">
                        ...
                        <? if ($hasChild) { ?>
                            <i class="material-icons" title="Подразделы" id="im<?= $row_menutree['cat_id'] ?>"
                               onClick="opentree(<?= $row_menutree['cat_id'] ?>, 2, 0);"><?= ($_COOKIE['amenu' . $parent] == 'Y') ? 'remove' : 'add' ?></i>
                        <? } ?>
                    </div>
                    ...
                    <div class="parent4">
                        <table border="0" cellpadding="0" cellspacing="0">
                            <tr>
                                <td align="center" class="lbr"><a href="editor.php?cat=<?php echo $row_menutree['cat_id']; ?>"
                                                                  target="Main"
                                                                  title="Редактировать содержимое раздела">
                                        <i class="material-icons">edit</i></a>
                                </td>
                                ...
                                if ($_SESSION['user_level'] < 1) {
                                    ...
                                    <td align="center" class="lbr">
                                        <a href="javascript:void(0)" title="Добавить подраздел">
                                            <i onClick="MM_openBrWindow('newcategory.php?parentid=<?= $row_menutree['cat_id']
                                            ?>&site_id=<?= $_SESSION['site_id'] ?>&cat_type=1','newcat',
                                                    'scrollbars=no,resizable=yes',
                                                    '720','450','true') " class="material-icons">playlist_add</i>
                                        </a>
