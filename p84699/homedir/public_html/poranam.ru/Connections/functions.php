                 ...
                 'Information')
{
	...
	try {
		...
		$mail->Host = 'p84699.hostnl02.fornex.host';                    // Set the SMTP server to send through smtp.mass.mail.mosreg.ru
		$mail->SMTPAuth = true;                                   // Enable SMTP authentication
		$mail->Username = 'info@poranam.ru';                     // SMTP username
		...
		//Recipients
		$mail->setFrom($sender, strtoupper($_SERVER['SERVER_NAME']));
...
function  getRegistry($type, $fields = ['id', 'field1'])
{
	//Считаем справочник
	$p = el_dbselect("SELECT ".implode(', ', $fields)." FROM catalog_{$type}_data WHERE active=1 ORDER BY field1, id",
        0, $p, 'result', true);
	$rp = el_dbfetch($p);
	$data = array();
	do {
...
function el_buildRegistryList($type, $selected = '', $firstEmpty = true, $exclude = array())
{
    $items = getRegistry($type);
...
function el_calcVoteResults($initId){
    $resvote = el_dbselect("SELECT field4 FROM catalog_initresult_data WHERE 
...
//Создаем поисковый подзапрос
    ...
    if($catalog_id == 'users'){
        ...
        if(intval($_GET['uid']) > 0){
            $active .= ' AND id = '.intval($_GET['uid']);
        }
    }
	$subquery = " active=$active $catQuery AND (site_id = 1 OR site_id = " . intval($_SESSION['view_site_id']) . ") ";
	...
	if(intval($_SESSION['user_level']) == 10 && $catalog_id == 'init') {
        $subquery .=  " OR field4 = '".$_SESSION['user_index'] . '_' . $_SESSION['user_id']."'";
    }
...
//Создаем подзапрос для сортировки
	...
	if (isset($_REQUEST['sort'])) {
		if (substr_count($_REQUEST['sort'], '|') > 0) {
			...
			$sortquery .= " ORDER BY " . implode(', ', $sfieldNum) ;
		} else {
...
function el_createUserGroup($region, $post_index, $number = 1){
	global $dbconn;
    $result = false;
	$ins = array(
		'site_id' => 1,
		'cat' => 420,
		'active' => 1,
		'path' => trim($post_index),
		'field1' => trim($post_index),
		'field2' => trim($number)
	);
	...
	if($res) {
		...
		if(el_dbnumrows($curators) > 0) {
			...
			//Делаем рассылку
			$regMessage = '<p>При регистрации нового пользователя создана новая группа №' . $post_index . '_' . $number.
			...
			$mailResult = el_mail(implode(',', $emails),
				...
				$letter_body, 'info@poranam.ru', 'html', 'smtp',
				'', 'info@poranam.ru', 'ПНМ «ПораНаМ»');
		}
	}
...
//Генация синонима ID пользователя для формирования реверральной ссылки
 ...
 *
 * @param number $score_id - id баллов из таблицы catalog_scores_data
 * @param number $user_id - id пользователя из таблицы catalog_users_data
 * @return bool
 */
function el_earnpoints($score_id, $user_id){
    $scores = el_dbselect("SELECT field3 FROM catalog_scores_data WHERE id = '$score_id' 
                                AND active = 1", 0, $scores, 'row', true);
    if(isset($score['field3'])){
