        ...
        <td>
            ...
            switch ($row_catalog['field14']) {
                case 1: //Редактировать только что созданную может зарегистрированный автор инициативы или КЦ
                    ...
                    break;
                case 2: //Остановить может Администратор
                    ...
                    break;
                case 3: //Завершенная инициатива
                    if ($_SESSION['user_level'] > 0) {
                        ?>
                        <button class="icon" title="Скачать ведомость"><span class="material-icons">get_app</span>
                        </button>
                        <?
										...
										<label class="container">
											<input type="radio" <?=($allowVote) ? '' : 'disabled'?>
												   data-id="<?= $row_catalog['id'] ?>" name="votes_vote"
													<?=(intval($rvote['field4']) == 0) ? 'checked' : ''?> value="<?=$rans['id']?>">
											<span class="checkmark radio"></span>
										</label>
									</div>
