...
//Дефолтные параметры
    ...
    Plugin.prototype.init = function (options) {
        ...
        let label = $this.data("label"),
            ...
            multi_bar_show = (typeof $this.data("multibarshow") !== "undefined")
                ? $this.data("multibarshow") : true,
        ...
        this.params[select_id] = {
            ...
            observer: observer,
            multi_bar_show: multi_bar_show
        };

        if (is_multiple !== "") {
            multi_bar = (this.params[select_id].multi_bar_show) ? '<div class="el_multi_bar" style="display: none">\n' +
    ...
    Plugin.prototype._removeValue = function (value) {
        let new_select_id = this.params[this.element.id].new_select_id,
            ...
            $orig_select = $select.closest(".el_data").prev("select");
        if ($select_row.is("div") && !is_disabled) {
            ...
            $orig_select.find("option[value='" + value + "']").attr("selected", false);
            $orig_select.trigger("el_select_change");
